...
#include "NavigationAndReportingClient.h"
#include "JausUtils.h"

using namespace JTS;
using namespace urn_jaus_jss_core_NavigationAndReportingClient_1_0;


NavigationAndReportingClient::NavigationAndReportingClient(unsigned int subsystem, unsigned short node, unsigned short component)
{
	jausRouter = new JausRouter(JausAddress(subsystem, node, component), ieHandler);
	
	/// Instantiate services
	NavigationAndReportingClientService* pNavigationAndReportingClientService = new NavigationAndReportingClientService(jausRouter);
	
	
	/// Add all the Services for the Component
	serviceList.push_back(pNavigationAndReportingClientService);
	
}

NavigationAndReportingClient::~NavigationAndReportingClient()
{
	Service* service;
...
void NavigationAndReportingClient::startComponent()
{
	Service* service;
...
void NavigationAndReportingClient::shutdownComponent()
{
	Service* service;
...
void NavigationAndReportingClient::processInternalEvent(InternalEvent *ie)
{
    bool done = false;
