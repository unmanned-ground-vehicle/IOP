...
#include "CapabilityDiscovery.h"
#include "JausUtils.h"

using namespace JTS;
using namespace urn_jaus_jss_mobility_CDServiceDef_1_0;


CapabilityDiscovery::CapabilityDiscovery(unsigned int subsystem, unsigned short node, unsigned short component)
{
	jausRouter = new JausRouter(JausAddress(subsystem, node, component), ieHandler);
	
	/// Instantiate services
	CapabilityDiscoveryServiceDefService* pCapabilityDiscoveryServiceDefService = new CapabilityDiscoveryServiceDefService(jausRouter);
	
	
	/// Add all the Services for the Component
	serviceList.push_back(pCapabilityDiscoveryServiceDefService);
...
void CapabilityDiscovery::startComponent()
{
	Service* service;
...
void CapabilityDiscovery::shutdownComponent()
{
	Service* service;
...
void CapabilityDiscovery::processInternalEvent(InternalEvent *ie)
{
    bool done = false;
